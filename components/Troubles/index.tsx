...
import { LegalTrouble, ResolutionOption } from '@/constants/Troubles';

const TroublesScreen: React.FC = () => {
  const { currentTroubles, resolveTrouble } = useBusinessContext();

  const renderResolutionOption = (trouble: LegalTrouble) => (option: ResolutionOption, index: number) => (
    <TouchableOpacity
    ...
    >
      ...
      <Text style={styles.resolutionButtonCost}>
        ${option.cost} / {option.influenceCost} influence
      </Text>
    </TouchableOpacity>
  );
  ...
  const renderTrouble: ListRenderItem<LegalTrouble> = ({ item: trouble }) => {
    ...
    return (
      <View style={styles.troubleItem}>
        ...
        <Text style={styles.troubleCost}>Cost: ${trouble.cost}, Influence: {trouble.influenceCost}</Text>
  ...
  return (
    <View style={styles.container}>
      ...
      {currentTroubles.length === 0 ? (
        <Text style={styles.noTroubles}>No current legal troubles. Great job!</Text>
      ) : (
